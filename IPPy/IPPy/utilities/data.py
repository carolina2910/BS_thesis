...
class ImageDataset(Dataset):
    ...
    def __init__(self, data_path: str, data_shape: int | None = None) -> None:
        ...
        self.img_name_list = sorted(
            glob.glob(os.path.join(self.data_path, "**", "*.png"), recursive=True)
        )

        if len(self.img_name_list) == 0:
            raise ValueError(
                f"No PNG images found under '{self.data_path}'.\n" \
                "If your images are in subfolders, they will now be discovered. " \
                "Check the provided path is correct and contains .png files."
            )
...
class TrainDataset(Dataset):
    ...
    def __init__(
        ...
        # recursively discover images in nested subfolders
        self.in_name_list = sorted(
            glob.glob(os.path.join(self.in_path, "**", "*.png"), recursive=True)
        )
        self.out_name_list = sorted(
            glob.glob(os.path.join(self.out_path, "**", "*.png"), recursive=True)
        )

        # Helpful errors if something is wrong with paths or dataset structure
        if len(self.in_name_list) == 0:
            raise ValueError(f"No PNG images found under input path '{self.in_path}'.")
        if len(self.out_name_list) == 0:
            raise ValueError(f"No PNG images found under output path '{self.out_path}'.")
        if len(self.in_name_list) != len(self.out_name_list):
            raise ValueError(
                f"Mismatch between number of input and output images: {len(self.in_name_list)} vs {len(self.out_name_list)}.\n"
                "Ensure the folder structures match and filenames correspond."
            )
